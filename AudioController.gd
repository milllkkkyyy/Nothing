...
func load_audio(path, loop = true):
...
func set_music(path, loop = true):
	music.stream = load_audio(path, loop)
	
func set_sfx(path, loop = true):
	sfx.stream = load_audio(path, loop)
